...
def get_args():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="If set, force recompute for existing feature files."
    )
    return parser.parse_args()


def compute_fbank_generic(args):
    ...
    if args.test:
        for partition in ["dev", "test"]:
            if (output_dir / f"cuts_{partition}.jsonl.gz").is_file() and not args.force:
